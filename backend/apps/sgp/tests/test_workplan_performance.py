...
Cobertura (tabela da issue):
    ...
    4. test_comando_reconcilia — Divergência forçada é corrigida (padrão do comando)
       test_comando_check_only_detecta_sem_corrigir — --check-only só detecta
    5. test_painel_queries_constantes — Mesmo nº de queries com 5 e com 30 Ações
       (bate no endpoint real do painel do PT, não em /api/v1/metas/)
...
def test_comando_check_only_detecta_sem_corrigir():
    acao = WorkPlanAcaoFactory()
    ActivityFactory(acao=acao, status="concluido")
    ...
    acao.refresh_from_db(fields=["quantidade_realizada"])
    assert acao.quantidade_realizada == 999
...
def test_painel_queries_constantes(auth_client):
    ...
    print(
        f"\n[RNF] queries do painel do PT: 5 ações={queries_com_5}, "
        f"30 ações={queries_com_30}"
    )
...
def test_painel_sob_500ms(auth_client, municipio):
    ...
    print(f"\n[RNF] painel do PT (105.000 atividades): {elapsed:.3f}s (limite 0.5s)")
...
def test_listagem_5000_upfs_sob_3s(auth_client, projeto, municipio, territory):
    ...
    print(f"\n[RNF] listagem de 5.000 UPFs paginada: {elapsed:.3f}s (limite 3s)")
